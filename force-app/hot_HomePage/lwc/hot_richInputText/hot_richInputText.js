...
export default class Hot_richInputText extends LightningElement {
    ...
    @api validate() {
        ...
        if (this.disableAdvancedTools || this.warnOnly) {
            ...
            return {
                isValid: false,
                errorMessage:
                    'Cannot Advance - Character Limit Exceeded: ' + this.characterCount + ' > ' + this.characterLimit
            };
            ...
            return {
                isValid: false,
                errorMessage:
                    'Cannot Advance - Invalid Symbols/Words Remain in Rich Text: ' + this.runningBlockedInput.toString()
            };
        } else {
            return { isValid: true };
        }
    ...
    startResize(event) {
        ...
        const wrapper = this.template.querySelector('[data-wrapper]');
        const editorHost = this.template.querySelector('lightning-input-rich-text');

        if (!wrapper || !editorHost) {
            return;
        }

        this.startY = event.clientY;
        this.startHeight = editorHost.getBoundingClientRect().height;

        // Mouse move listener
        const mouseMove = (e) => {
            const delta = e.clientY - this.startY;
            const newHeight = Math.max(80, this.startHeight + delta);

            wrapper.style.setProperty('--rtf-size', `${newHeight}px`);
        };
    ...
    handleTextChange(event) {
        ...
        if (this.symbolsNotAllowed !== undefined || this.wordsNotAllowed !== undefined) {
            ...
            this.interimValue = this.interimValue.replace(/(<([^>]+)>)/gi, '');
        ...
        if (this.excludeHTMLCharacterCount) {
            ...
            this.characterCount = this.richText.replace(/<[^>]*>/gi, '').length;
        }
